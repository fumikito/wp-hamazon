  ...
  paginate() {
    ...
    return (
      <div className="hamazon-pagination">
        {(() => {
          return showPrevious ? (
            <button key="hamazon-pagination-item-first" className="hamazon-pagination-item" onClick={(event) => {
              this.setState({
                curPage: this.state.curPage - 1,
              }, () => {
                this.submitHandler(event);
              });
            }}>{HamazonEditor.previousPage}</button>
          ) : null;
        })()}
        {pages.map((p, index) => {
        ...
        {(() => {
          return showNext ? (
            <button key="hamazon-pagination-item-last" className="hamazon-pagination-item" onClick={(event) => {
              this.setState({
                curPage: this.state.curPage + 1,
              }, () => {
                this.submitHandler(event);
              });
            }}>{HamazonEditor.nextPage}</button>
          ) : null;
        })()}
      </div>
    );
